...
public:
	...
	CLCodeAttribute(int attribute_name_index, long long attribute_length, int max_stack, int max_locals, long long code_length, std::vector<int> code,
		int exception_table_length, std::vector<CLExceptionInfo> exception_table, int attributes_count, std::vector<CLAttributeInfo> attributes);
	void write(CLOutputStream out);
private:
...
public:
	...
	CLElementValue(int numValues, std::vector<CLElementValue> values);
	void write(CLOutputStream out);
private:
};
